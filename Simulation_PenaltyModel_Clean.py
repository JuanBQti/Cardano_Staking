...
# Model with Payment conditional on being in the Winning Side and Penalty conditional on being in the Losing Side
...
b   = 0                               #fixed penalty paid when on the losing side (k < n)
...
def compute_equilibrium(n, b=b, k_range_winning=None, k_range_losing=None, K_R=None, find_all_roots=True):
    ...
    # First, check for corner solutions by examining FOC at boundaries and interior
    # Check FOC at x=0 and x=0.5 first (boundaries)
    foc_at_0 = None
    foc_at_05 = None
    ...
    # Check boundary conditions (but don't return yet - we need to check for interior roots too)
    # If FOC < 0 at x=0, profit is decreasing from 0 -> x=0 is a potential corner
    # If FOC > 0 at x=0.5, profit is increasing to 0.5 -> x=0.5 is a potential corner
    potential_corners = []
    if foc_at_0 is not None and foc_at_0 < 0:
        profit_at_0 = profit(0.0)
        potential_corners.append((0.0, profit_at_0, 'corner'))
    
    if foc_at_05 is not None and foc_at_05 > 0:
        profit_at_05 = profit(0.5)
        potential_corners.append((0.5, profit_at_05, 'corner'))
    
    # Check interior to see if we can identify additional corners
    # Sample FOC at several interior points to determine if it's always positive or always negative
    ...
    if len(foc_samples_clean) > 0:
        ...
        # If FOC > 0 for all interior x, profit is increasing in interior -> corner at x=0.5
        if min_foc > 0:
            profit_at_05 = profit(0.5)
            # Add to potential corners if not already there
            if not any(abs(x - 0.5) < 1e-6 for x, _, _ in potential_corners):
                potential_corners.append((0.5, profit_at_05, 'corner'))
        
        # If FOC < 0 for all interior x, profit is decreasing in interior -> corner at x=0
        if max_foc < 0:
            profit_at_0 = profit(0.0)
            # Add to potential corners if not already there
            if not any(abs(x - 0.0) < 1e-6 for x, _, _ in potential_corners):
                potential_corners.append((0.0, profit_at_0, 'corner'))
    ...
    # Add potential corner solutions (identified at boundaries)
    # These are valid equilibria regardless of whether we found interior roots
    for corner_x, corner_profit, corner_type in potential_corners:
        # Check if we already have this in the list (avoid duplicates)
        if not any(abs(x - corner_x) < 1e-6 for x, _, _ in equilibria_list):
            equilibria_list.append((corner_x, corner_profit, corner_type))
    
    # If no roots found and no corners identified yet, do a final check
    if len(equilibria_list) == 0:
        # Re-check boundaries
        try:
            foc_at_0_check = first_order_condition(0.0)
            if foc_at_0_check < 0:
                profit_at_0 = profit(0.0)
        ...
        try:
            foc_at_05_check = first_order_condition(0.5)
            if foc_at_05_check > 0:
                profit_at_05 = profit(0.5)
...
for n in n_list:
    ...
    if eq is not None:
        # Handle return format: can be single x value or list of tuples
